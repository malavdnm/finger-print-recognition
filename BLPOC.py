...
def BLPOC(input_image, reg_image_corrected, k1, k2):
	...
	rows, cols = cross_phase_spec.shape
	k1 = rows//6
	k2 = cols//6
	m1 = rows//2
	m2 = cols//2
	...
	blpoc_fft = cross_phase_spec[m1-k1:m1+k1,m2-k2:m2+k2]
	...
	blpoc_ifft[np.where(blpoc_ifft==np.max(blpoc_ifft))] = -10000
	second_lrg = np.amax(blpoc_ifft)
